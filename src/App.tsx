...
const DEFAULT_COLLATERAL_ASSETS = ["BTC", "SOL", "USDC", "USDT"];
const DEFAULT_BORROW_ASSETS = ["USDC", "USDT", "BTC", "SOL"];
...
export function App() {
  ...
  async function handleCreateLoan(event: FormEvent<HTMLFormElement>) {
    ...
    if (!collateralAsset || !borrowAsset) {
      setTrackingError("Selected pools are not supported by instant loans.");
      return;
    }
    ...
    try {
      const loan = await liquidiumClient.instantLoans.create({
...
async function fetchMarketData(): Promise<{ pools: Pool[]; prices: AssetPrices }> {
...
async function fetchLoanTracking(shortRef: string): Promise<TrackingResult> {
...
function formatScaledRate(rate: bigint, decimals: bigint): string {
...
function formatQuoteErrors(ltv: LtvCalculation): string {
...
function getPoolById(pools: Pool[], poolId: string): Pool | undefined {
  return pools.find((pool) => pool.id === poolId);
}

function chooseDefaultPool(pools: Pool[], preferredAssets: string[]): Pool | undefined {
...
function isInstantLoanAsset(asset: string): asset is InstantLoanAsset {
